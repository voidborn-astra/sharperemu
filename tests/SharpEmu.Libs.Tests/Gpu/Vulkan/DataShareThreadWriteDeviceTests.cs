...
public sealed class DataShareThreadWriteDeviceTests(HeadlessVulkanFixture fixture) : IClassFixture<HeadlessVulkanFixture>
{
    [Theory]
    ...
    public void ThreadOffsetWriteUsesScalarBaseOffsetAndExecutionMask(uint waveSize, uint scalarBase,
        uint byteOffset, bool maskOddLanes)
    {
        ...
        {
            var expected = maskOddLanes && (lane & 1) != 0 ? Gen5DataShareThreadWriteTests.InitialWord : 1000u + (uint)lane;
